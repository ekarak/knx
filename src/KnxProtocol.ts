...
export interface KnxProtocol extends BinaryProtocol {
...
const KnxProtocol: KnxProtocol = new BinaryProtocol() as KnxProtocol;
// defaults
...
KnxProtocol.lengths["ConnState"] = (value: Datagram["connstate"]) => (value ? 2 : 0);
...
KnxProtocol.lengths["TunnState"] = (value: Datagram["tunnstate"]) => (value ? 4 : 0);
...
KnxProtocol.define("APDU", {
  ...
  write(value: Datagram["cemi"]["apdu"]) {
    ...
    let word =
      value.tpci * 0x400 + APCICODES.indexOf(value.apci) * 0x40;
    //
    if (total_length == 3) {
