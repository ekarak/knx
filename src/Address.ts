...
export function parse(
	...
	if (group && twoLevelAddressing) {
		...
		if (r(hinibble, 31))
			throw Error(`Invalid KNX 2-level main group: ${addr}`)
		if (r(midnibble, 2047))
			throw Error(`Invalid KNX 2-level sub group: ${addr}`)
	...
	if (group) {
		...
		if (r(hinibble, 31))
			throw Error(`Invalid KNX 3-level main group: ${addr}`)
		if (r(midnibble, 7))
			throw Error(`Invalid KNX 3-level mid group: ${addr}`)
		if (r(lonibble, 255))
			throw Error(`Invalid KNX 3-level sub group: ${addr}`)
		address.writeUInt8((hinibble << 3) + midnibble, 0)
		address.writeUInt8(lonibble, 1)
		return address
	}
	// 3 level physical address
	if (r(hinibble, 15)) throw Error(`Invalid KNX area address: ${addr}`)
	if (r(midnibble, 15)) throw Error(`Invalid KNX line address: ${addr}`)
	if (r(lonibble, 255)) throw Error(`Invalid KNX device address: ${addr}`)
